...
async def quote_cmd(client: Client, message: types.Message):
    ...
    params = {
        'messages': [
            ...
            for msg in messages if not msg.empty
        ],
        'quote_color': '#162330',
        'text_color': '#fff'
    }
...
async def fake_quote_cmd(client: Client, message: types.Message):
    ...
    params = {
        'messages': [
            await render_message(client, q_message)
        ],
        'quote_color': '#162330',
        'text_color': '#fff'
    }
    ...
    text = (
        ...
        if reply.new_chat_members
            ...
            if reply.left_chat_member.id == reply.from_user.id
                ...
                get_full_name(reply.left_chat_member)
            )
        )
...
modules_help.update({'squotes': 'q [count] [args] - Generate a quote]\n'
